...
Pete can bake (integer). For simplicity there are no units
for the amounts (e.g. 1 lb of flour or 200 g of sugar are simply 1 or 200).
Ingredients that are not present in the objects, can be considered as 0."""


def cakes(recipe, available):
    return min(int(available.get(mater, 0)/recipe[mater])for mater in recipe)


print(cakes({"flour": 500, "sugar": 200, "eggs": 1, "apple": 2},
